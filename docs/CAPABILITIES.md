...
| 9 | **Least-privilege tool scoping** | Production connectors are mounted read-only and the single write path is gated by require_approval_for_tools. The agent cannot reach production unsupervised. | `config` | `enable_tools / require_approval_for_tools on the agent spec` | Permissions view in the run header |
...
| 13 | **Human approval checkpoint** | Applying to production is the only gated action, and it is gated on a certificate. | `stream` | `tool.approval_required` | The Certificate card |
...
| 19 | **HTTP API + SDK** | A migration PR opens an AIRLOCK change on its own. Nobody types anything. | `runtime` | `session created through the HTTP API by the webhook` | The started-by-webhook badge on the run |
