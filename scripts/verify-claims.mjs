...
const CLAIMS = [
  ...
  },
  {
    ...
    sees: 'Twelve tools; this is the one that records what was resolved and where from.',
  },
  {
    group: 'The agent',
  ...
  },
  {
    ...
    run: 'node --test packages/contract/test/connection.test.mjs',
    sees: 'The generated role grants no write privilege on any GRANT line, and never invents a password.',
  },
  {
    group: 'Real databases',
    claim: 'The connection string never survives a round trip into a transcript, a log, an error message or a stack trace.',
    file: 'packages/contract/src/connection.ts',
    anchor: 'export function redactDeep',
    run: 'node --test packages/contract/test/connection.test.mjs',
    sees: 'A realistic transcript is scanned for the password: zero hits, and the host deliberately survives so errors stay diagnosable.',
  },
  {
    group: 'Real databases',
    claim: 'Nothing on the connect-to-apply path can import a seed, a fixture, a mock or a generator. The boundary is enforced, not documented.',
    file: 'scripts/check-no-simulation.mjs',
    anchor: 'const ENTRY_POINTS = [',
    run: 'npm run check:simulation',
    sees: 'The import graph is walked from every module on that path; a violation fails the build with the chain that reached it.',
  },
  {
    group: 'The agent',
