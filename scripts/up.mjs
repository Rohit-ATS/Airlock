...
if (!envHas('AIRLOCK_MCP_HTTP_TOKEN') && !process.env.AIRLOCK_MCP_HTTP_TOKEN) {
  const token = `amcp_${randomBytes(32).toString('hex')}`;
  const block =
    '\n## HTTP MCP transport bearer token. Generated by `npm run up`.\n' +
    '## TrueForge sends it when it calls AIRLOCK over remote MCP; distinct from the console write token.\n' +
    `AIRLOCK_MCP_HTTP_TOKEN=${token}\n`;
  fs.appendFileSync(envFile, envText.endsWith('\n') ? block : `\n${block}`);
  envText = fs.readFileSync(envFile, 'utf8');
  ok('MCP HTTP token generated');
} else {
  ok('MCP HTTP token present');
}
